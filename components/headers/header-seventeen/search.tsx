...
const Search = ({ search, setSearch }: any) => {
    ...
    return (
        <motion.div
            ...
            className="shadow-xl rounded-md bg-white w-[80%] md:w-[60%] lg:w-[50%] top-2 z-50 left-0 right-0"
        >
            <h3 className="text-lg font-semibold mx-6 py-1">
                {result?.length} results for{' '}
                <span className="font-bold text-red-400">{search}</span>
            </h3>
            <div className=" grid grid-cols-2 gap-2 my-4 justify-center w-full">
                {result?.map((res, idx) => (
        ...
        >
            ...
            <div className="flex flex-col justify-between p-4 leading-normal">
                <Link
                    ...
                    onClick={() => setSearch('')}
                    className="mb-2 text-sm font-semibold tracking-tight text-gray-900 dark:text-white"
                >
