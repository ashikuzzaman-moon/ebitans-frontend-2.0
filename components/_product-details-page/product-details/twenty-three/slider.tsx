    ...
    <div>
      ...
      <div className="grid grid-cols-1 w-full gap-5 ">
        ...
        <div className="relative h-full w-full order-last group">
            ...
            {images?.slice(0, 10).map((item, index) => (
              <div key={index} className="focus:outline-none px-2">
                <img
                  ...
                  className={`${
                    activeMbl === index ? "active-img opacity-50 " : ""
                  } max-h-[200px] w-full bg-gray-100 border border-transparent lg:cursor-pointer`}
                  src={productImg + item}
                  alt=""
                />
              </div>
          ...
          {images.length > 4 && (
            <div className="lg:opacity-0 lg:group-hover:opacity-100 duration-500">
              <BsFillArrowDownSquareFill
                className="absolute -rotate-90 right-0 top-[50%] -translate-y-[50%] z-10 text-3xl arrow-slick-color lg:cursor-pointer"
                onClick={() => gotoNext()}
              />
              <BsFillArrowUpSquareFill
                className="absolute -rotate-90 left-0 z-10 text-3xl top-[50%] -translate-y-[50%] arrow-slick-color lg:cursor-pointer"
                onClick={() => gotoPrev()}
              />
            </div>
          )}
        ...
        <div className="z-[1] w-full">
          <div className="h-full w-full relative">
            <ImageZoom
              img={
                productImg +
                images[colorid && images?.length > colorid ? colorid : id]
              }
            />
            {/* <img src={productImg + images[id]} alt="" className='h-auto min-w-full' /> */}
            <div
              onClick={openModal}
              className="z-[2] search-color md:flex hidden lg:cursor-pointer absolute top-3 right-3 cart-btn h-8 w-8 rounded-full  items-center justify-center"
            >
              ...
              >
                <Dialog.Panel className="h-full w-max mx-10 z-50 transform bg-transparent  text-left align-middle transition-all">
                  <div className="flex md:flex-row items-center flex-col-reverse gap-10 ">
                    ...
                    <div className="h-auto w-[450px] bg-gray-100 overflow-hidden">
                      <img
