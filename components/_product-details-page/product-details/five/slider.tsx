...
export const HSlider = ({ product }: any) => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-5 gap-y-3 justify-start sm:flex-row gap-x-5 overflow-hidden">
        {/* for large device  */}
        <div className="relative flex flex-col justify-center items-center w-full sm:max-h-[450px] xl2:max-h-[600px] overflow-hidden">
          <Slider
            {...settings}
            ref={customeSlider}
        ...
        {/* <div className='relative sm:hidden'>
                    <Slider {...settingsSmall} ref={customSlider} className='relative group h-full w-full '>
                        {
                            images?.slice(0, 10).map((item, index) =>
                                <div key={index}>
                                    <img onClick={() => { setActiveMbl(index); setId(index); }} className={`${activeMbl === index ? 'active-img-mbl ' : ''} h-auto min-w-full bg-gray-100 border border-transparent focus:outline-none`} src={productImg + item} alt="" />
                                </div>
                            )
                        }
                    ...
                    {images?.length > 4 && <div>
                        <BsFillArrowDownSquareFill className='absolute -rotate-90 right-0 top-[50%] -translate-y-[50%] z-10 text-3xl arrow-slick-color' onClick={() => goNext()} />
                        <BsFillArrowUpSquareFill className='absolute -rotate-90 left-0 z-10 text-3xl top-[50%] -translate-y-[50%] arrow-slick-color' onClick={() => goPrev()} />
                    </div>}
                </div> */}
