...
import { addToCartList } from '@/redux/products/productSlice';
...
import { makeid } from '@/helper/getBakedId';
...
const Details = ({
    ...
    const storedata = useSelector((state: RootState) => state.store.data); // Access updated Redux state
    const { design, headersetting, store_id } = storedata || {};
        ...
        try {
            ...
            if (referral && productID) {
                ...
                localStorage.setItem(
                    'referralObj',
                    JSON.stringify(referralObj)
                );
            }
        } catch (error) {
            console.error(
                'Error parsing referralObj from localStorage:',
                error
            );
